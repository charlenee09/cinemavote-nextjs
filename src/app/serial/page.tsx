...
// Ambil data live lewat jembatan api
  useEffect(() => {
    movieApi.getAllMovies()
      ...
      .catch((err) => console.error("Gagal konek backend di halaman serial:", err));
  }, []);
  ...
  const handleVote = async (movieId: number) => {
    try {
      ...
      setDbSerials((prevSerials) =>
        prevSerials.map((movie) =>
      ...
      console.error("Error pas vote di halaman serial:", error);
    }
  };

  // 🛠️ LOGIKA FILTER KONTEN AMAN & DEFENSITIF
  const filteredSerials = Array.isArray(dbSerials)
    ? dbSerials.filter((movie) => {
        // 1. Ambil data yang berjenis serial (mengecek kolom tipe/kategori film di database kamu)
        // Catatan: Jika kolom category dipakai untuk penanda "serial", pastikan tidak bentrok dengan genre.
        const isSerial = movie?.category?.toLowerCase() === "serial" || movie?.type?.toLowerCase() === "serial";
        ...
        return isSerial && matchSearch && matchGenre && matchRating;
      })
    : [];
    ...
    <section className="px-10 min-h-screen bg-black pt-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {filteredSerials.length > 0 ? (
          filteredSerials.map((movie) => (
            <MovieCard 
              key={movie.id} 
              movie={movie} 
              onOpenDetail={(data) => setSelectedMovie(data)} 
              onVote={handleVote} // 👈 Oper fungsi vote ke MovieCard halaman serial
            />  
          ))
        ) : (
          <div className="col-span-full text-center py-20 text-gray-500 italic">
            {searchTerm !== "" ? (
              <span>Yah, maaf drama "{searchTerm}" tidak dapat ditemukan... 🎬</span>
            ) : (
      ...
      {selectedMovie && (
        <DetailModal 
          ...
          onVote={handleVote} // 👈 Oper fungsi vote juga ke DetailModal halaman serial
        />
      )}
    </section>
  );
