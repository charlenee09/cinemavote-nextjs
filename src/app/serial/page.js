...
export default function SerialPage() {
  ...
  // LOGIKA FILTER KONTEN - SERIAL
  const filteredSerials = movies.filter((movie) => {
    ...
    <section className="px-10 min-h-screen bg-black pt-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {filteredSerials.length > 0 ? (
          ...
          /* LOGIKA PEMISAH PESAN KOSONG DI HALAMAN SERIAL */
          <div className="col-span-full text-center py-20 text-gray-500 italic">
            {searchTerm !== "" ? (
              // Jika kosong karena lagi ngetik di search box
              <span>Yah, maaf drama "{searchTerm}" tidak dapat ditemukan... 🎬</span>
            ) : (
              // Jika kosong murni karena filter dropdown Genre
